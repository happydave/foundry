...
- **Estimator** `/internal/estimator/`
  - Inputs: `ModelSpec` (file size, layer count, KV head count, head dim, max context, plus sliding-window fields: window size, SWA head dim, and global/SWA layer and KV-head counts), context size, KV cache type, `nParallel` (number of parallel slots), in-use VRAM bytes; reads `/sys/class/drm/card*/device/mem_info_vram_{total,used}` and `/proc/meminfo` via `resources.go`. `QueryHardware` (`hardware.go`) additionally reads per-card memory pools (`mem_info_gtt_*`, `mem_info_vis_vram_*`, `mem_info_preempt_used`) and best-effort telemetry (`gpu_busy_percent`, hwmon `temp1_input`/`power1_average`, `pp_dpm_sclk`). `QueryProcessGPUMemory` (`procgpu.go`) reads per-process GPU memory from `/proc/<pid>/fdinfo` for a **caller-supplied** PID list — the caller passes the process manager's loaded-model PIDs, so the estimator still takes no dependency on `/internal/processmanager/`
  - Outputs: `ForwardResult` (weight cost, KV cost, total cost, feasibility); `InverseResult` (maximum fitting context size); `GPUInfo` (per-card VRAM plus `Pools` and `Telemetry`); `ProcessGPUMemory` (per-PID resident VRAM/GTT attributed to a card)
